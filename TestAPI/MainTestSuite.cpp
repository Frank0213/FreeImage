...
int main(int argc, char *argv[]) {
	...
	FREE_IMAGE_FORMAT fif = FreeImage_GetFileType("125.bmp");
	dib = FreeImage_Load(fif, "125.bmp", 0);
	BYTE *aa = new BYTE(0);
	aa = FreeImage_GetBits(dib);
	//assert(dib != NULL);
	BYTE *table = new BYTE(0);
	for (size_t i = 0; i < 600; i++)
	{
		FreeImage_SetPixelIndex(dib, 300, i, table);

	}
	FreeImage_Save(fif, dib, "1252.bmp", 0);
